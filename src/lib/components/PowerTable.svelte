<script context="module" lang="ts">
	...
	export interface Options {
		...
		segments?: Record<
			string,
			Array<'settings' | 'search' | 'pagination' | 'table' | 'dropdown' | 'stats'>
		>;
...
<script lang="ts">
	...
	function initialize(
		...
		action: { render?: boolean; preserveFilters?: boolean } = {
			render: true,
    ...
    */
	...
	function applyFilters() {
		...
		if (searchObj.value) {
			...
			if (
				typeof (options.userFunctions?.customSearch ?? null) === 'function' &&
		...
		Object.entries(filterObj).forEach(([key, filter]) => {
			...
			if (filter.value?.length) {
				...
				if (
					typeof (correspondingInstruct?.userFunctions?.customFilter ?? null) === 'function' &&
					correspondingInstruct?.userFunctions?.customFilter !== undefined
				) {
					let customFilterResult = correspondingInstruct.userFunctions.customFilter(
						matchedData,
						filter.value
					);
				...
				if (customFilterContinue !== false) {
					...
					if (regexParts) {
						...
						try {
							...
							// @ts-ignore Shhhh!
							matchedData = matchedData.filter((d) =>
								new RegExp(regexParts.pattern, flags).test(d[key])
							);
						} catch (e) {}
					}
	...
	function applyPagination() {
		...
		p.firstShownRow = Math.min(
			p.filteredRows,
			(options?.currentPage! - 1) * options?.rowsPerPage! + 1
		);
		...
		if (
			options.currentPage! <= options.paginationBlock! + 1 ||
		...
		if (
			typeof (options.userFunctions?.customParse ?? null) === 'function' &&
	...
	function manualRowEdit(e: Event, index: number) {
		let textareaEls = (<HTMLInputElement>e.target)
			.closest('tr')
			?.querySelectorAll<HTMLInputElement>('textarea[data-name=edit-textarea]');
	...
	function rowClicked(e: Event, index: number) {
		...
		if ((<HTMLInputElement>e.target).dataset?.name === 'edit-submit') {
			...
			textareaEls!.forEach((textareaEl) => {
				data[pageData[index][dataIdKey]][(<HTMLInputElement>textareaEl)?.dataset?.key ?? ''] =
					(<HTMLInputElement>textareaEl)?.value ?? '';
			});
	...
	export function closePopUps({ target }: Event) {
		const poppedEls = Array.from(
			document.querySelectorAll('[data-popped=true]') as NodeListOf<HTMLElement>
		);
	...
	export function toggleMenu(e: Event) {
		let menuEl = (<HTMLInputElement>e?.target)?.parentElement?.querySelector<HTMLElement>(
			'div[data-name=menu]'
		);
	...
	export function getData(removeMetadata = true) {
		...
		if (removeMetadata) {
			...
			exportInstructs = exportInstructs.filter(
				(instruct) => !specialInstructs.hasOwnProperty(instruct.key)
			);
		}
